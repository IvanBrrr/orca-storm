...
case "$entrypoint_kind" in
  ...
  launcher)
    entrypoint=("$app_root/resources/bin/orca-storm")
    ;;
...
if [[ "$entrypoint_kind" == appimage ]]; then
  registered_cli="$HOME/.local/bin/orca-storm"
  expected_target="$XDG_CACHE_HOME/orca-storm/appimage/launcher/orca-storm"
...
for shutdown_poll in {0..50}; do
  ...
  owned_residue=$(ps -eo pid=,ppid=,stat=,args= | awk -v state="$state_dir" \
    '($0 ~ state || $0 ~ /\/artifacts\/root\/orca-storm/ || $0 ~ /[X]vfb :99 /) && $0 !~ /awk -v state=/ {print}' || true)
