...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('IvanBrrr/orca-storm')
    expect(getReleaseRepoForChannel('daily')).toBe('IvanBrrr/orca-storm')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('IvanBrrr/orca-storm')
    expect(getReleaseRepoForChannel('stable')).toBe('IvanBrrr/orca-storm')
    expect(getReleaseRepoForChannel('rc')).toBe('IvanBrrr/orca-storm')
  })
  ...
  it('builds release-notes links against the repo that published the version', () => {
    expect(getReleaseNotesUrlForVersion('1.4.160-hourly.202607281400')).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases/tag/v1.4.160-hourly.202607281400'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases/tag/v1.4.160-daily.202607281300'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160')).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases/tag/v1.4.160'
    )
    expect(getReleaseNotesUrlForVersion('v1.4.160-rc.3')).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases/tag/v1.4.160-rc.3'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe(
      'https://github.com/IvanBrrr/orca-storm/releases'
    )
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/IvanBrrr/orca-storm/releases/tag/v${version}`,
      installerUrl: null
    })
