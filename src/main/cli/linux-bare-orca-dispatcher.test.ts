...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  ...
  // The bundled orca-storm launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'orca-storm'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareOrcaDispatcher', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    expect(result.target).toBe(join(resourcesPath, 'bin', 'orca-storm'))
  })

  it('writes an executable bare-orca dispatcher that execs the bundled orca-storm launcher', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    ...
    const expectedTarget = join(resourcesPath, 'bin', 'orca-storm')
    expect(result.state).toBe('installed')
    ...
    async () => {
      ...
      const result = await installLinuxBareOrcaDispatcher({
        ...
        appImageExtractRunner: async (_appImagePath, cwd) => {
          ...
          await writeFile(join(launcherDir, 'orca-storm'), '', { encoding: 'utf8', mode: 0o755 })
        }
      })
  ...
  it('skips when the bundled orca-storm launcher is missing from the build', async () => {
...
async function writePayload(cwd: string): Promise<void> {
  ...
  await writeFile(join(launcherDirectory, 'orca-storm'), '#!/usr/bin/env bash\n', { mode: 0o755 })
}
