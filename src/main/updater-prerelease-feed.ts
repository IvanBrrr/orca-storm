...
const ATOM_FEED_URL = 'https://github.com/IvanBrrr/orca-storm/releases.atom'
const RELEASES_DOWNLOAD_BASE = 'https://github.com/IvanBrrr/orca-storm/releases/download'
...
const TAG_HREF_RE = /href="https:\/\/github\.com\/IvanBrrr\/orca-storm\/releases\/tag\/([^"]+)"/g
...
async function getReleaseAssetReadiness(tag: string, assetName: string): Promise<ReleaseReadiness> {
  ...
  const isGitHubReleaseAsset =
    ...
    (isRelativeAsset ||
      /^https:\/\/github\.com\/IvanBrrr\/orca-storm\/releases\/download\//i.test(assetName))
