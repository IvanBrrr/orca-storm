...
> {
  ...
  try {
    const viewerResponse = await requestWithCredentials(baseUrl, apiToken, 'users/me/')
    if (!isPlaneJsonRecord(viewerResponse)) {
      throw new Error('Plane returned an invalid user profile.')
    }
    const viewer = toPlaneViewer(viewerResponse)
...
export async function testConnection(
  ...
  try {
    const viewerResponse = await planeRequest(client, 'users/me/')
    if (!isPlaneJsonRecord(viewerResponse)) {
      throw new Error('Plane returned an invalid user profile.')
    }
    const viewer = toPlaneViewer(viewerResponse)
