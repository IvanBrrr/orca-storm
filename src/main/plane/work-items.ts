...
function droppedRelationships(raw: unknown, mapped: PlaneWorkItem): boolean {
  const record = isPlaneJsonRecord(raw) ? raw : {}
...
export async function listWorkItems(
  ...
  if (!needsFallback) {
    return { items: mapped.filter((item): item is PlaneWorkItem => item !== null), truncated }
  }
...
export async function searchWorkItems(
  ...
  const payload = await planeRequest(
    client,
  ...
  const rows = isPlaneJsonRecord(payload) && Array.isArray(payload.issues) ? payload.issues : []
  return rows
...
export async function listComments(
  ...
  const { items } = await listAllPages((cursor) =>
    planeRequest(
      client,
