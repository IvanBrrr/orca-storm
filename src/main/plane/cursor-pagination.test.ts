...
describe('listAllPages', () => {
  it('follows next_cursor until the data ends', async () => {
    ...
    await expect(listAllPages(fetchPage)).resolves.toEqual({
  ...
  it('stops and reports truncation at maxItems', async () => {
    ...
    await expect(listAllPages(fetchPage, { maxItems: 2 })).resolves.toEqual({
      items: ['a', 'b'],
      truncated: true
    })
  ...
  it('reports truncation when the page bound is hit', async () => {
    ...
    const result = await listAllPages(fetchPage, { maxPages: 2, maxItems: 100 })
    expect(result.truncated).toBe(true)
    expect(fetchPage).toHaveBeenCalledTimes(2)
  })

  it('accepts a bare array, which small collections return', async () => {
    await expect(listAllPages(async () => ['only'])).resolves.toEqual({
      items: ['only'],
      truncated: false
    })
  })
  ...
  it.each([
    ...
    await expect(listAllPages(fetchPage)).resolves.toEqual({ items: [], truncated: false })
  })

  it('treats a missing results field as an empty page', async () => {
    await expect(listAllPages(async () => ({}))).resolves.toEqual({
      items: [],
      truncated: false
    })
  })
