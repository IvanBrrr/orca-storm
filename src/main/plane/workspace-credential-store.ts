...
function readWorkspaceFileFromDisk(): PlaneWorkspaceFile {
  ...
  try {
    const decoded: unknown = JSON.parse(readFileSync(path, { encoding: 'utf-8' }))
    const parsed = isPlaneJsonRecord(decoded) ? decoded : {}
...
function resolveSelection(
  ...
  const activeWorkspaceId = has(activeCandidate) ? activeCandidate : (workspaces[0]?.id ?? null)
